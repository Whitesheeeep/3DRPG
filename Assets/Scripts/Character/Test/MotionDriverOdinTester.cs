    ...
    public sealed class MotionDriverOdinTester : MonoBehaviour
    {
        ...
        public void VerifyArbitration()
        {
            ...
            try
            {
                ...
                driver.SubmitUpdate(walk, UpdateMotionSubmission.TranslationOnly(Vector3.right * 10f));
                driver.ResolveUpdateMotion();
                ExpectDelta(probe, before, Vector3.zero, "高优先级技能未在 Update 提交时不回退 Locomotion");
                skill.Dispose();
                skill.Dispose();
                before = probe.transform.position;
                ...
                before = probe.transform.position;
                driver.SubmitUpdate(walk, UpdateMotionSubmission.TranslationOnly(Vector3.right * 2f));
                driver.ResolveUpdateMotion();
                ExpectDelta(probe, before, Vector3.right * 2f, "Update 阶段提交与结算");
                before = probe.transform.position;
                driver.ResolveFixedMotion();
