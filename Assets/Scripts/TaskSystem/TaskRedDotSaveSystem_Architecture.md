...
本节记录当前基础版的产品边界。核心剧情任务的后续需求不在此节追加，避免把尚未实现的阶段、导航和资源占用误认为现有 API 已经存在。
...
- 可领取状态不产生任务红点；任务列表直接展示 `Claimable`，领奖成功后清理活动记录。
- 同帧多次业务变化只在批处理后通知最终值。
- 值未变化时不发送重复通知。
...
- 日常、周常、周期重置和无限重复任务。
- 任务失败、放弃、重新接取和任务资源占用冲突。
- 目标 OR 分组、嵌套表达式和阶段式目标；阶段式目标由新需求文档定义为下一阶段能力。
- 自动选择下一个追踪任务。
- 部分奖励发放、奖励回滚和邮箱补发。
- 红点轮询刷新。
...
下一阶段的任务产品需求、接口方向和验收场景见 [`TaskSystem_Requirements.md`](TaskSystem_Requirements.md)。实现时应继续遵守本文件定义的 Manager/System、事件、红点和存档边界。

1. **关键节点自动保存清单**：由存档系统和任务需求共同明确任务领奖完成、阶段切换、场景切换和暂停菜单退出等触发点。
2. **模块恢复依赖顺序**：按新需求文档的恢复时序，在玩家、背包、场景等业务模块成型后补充具体拓扑顺序。
