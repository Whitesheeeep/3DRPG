...
{
    /// <summary>覆盖在 BagWindow 上方的武器或圣遗物装备培养窗口。</summary>
    public sealed class EquipmentDevelopmentWindow : WindowBase, IWindowWithOpenContext<EquipmentDevelopmentOpenContext>
    {
        #region 依赖与状态

        private EquipmentDevelopmentWindowDataComponent data;
        private EquipmentDevelopmentWindowController controller;
        ...
        {
            data = GameObject.GetComponent<EquipmentDevelopmentWindowDataComponent>();
            if (data == null) throw new InvalidOperationException("[EquipmentDevelopmentWindow] 根节点缺少 EquipmentDevelopmentWindowDataComponent。");
            data.ValidateConfiguration();
            FullScreenWindow = data.IsFullWindow;
            SetDoAnimation(data.DoAnimation);
            base.OnAwake();
            controller = GameObject.GetComponent<EquipmentDevelopmentWindowController>();
            if (controller == null) throw new InvalidOperationException("[EquipmentDevelopmentWindow] 根节点缺少 EquipmentDevelopmentWindowController。");
            controller.Initialize(data);
        }

        /// <summary>应用本次打开目标并注册培养窗口 Esc Command。</summary>
        /// <param name="context">装备实例打开上下文。</param>
        public void ApplyOpenContext(EquipmentDevelopmentOpenContext context)
        {
        ...
        public override void OnShow()
        {
            ...
            escCommandRegistration = escapeArchitecture.SendCommand(
                new RegisterEscCommand(new CloseEquipmentDevelopmentWindowCommand()));
