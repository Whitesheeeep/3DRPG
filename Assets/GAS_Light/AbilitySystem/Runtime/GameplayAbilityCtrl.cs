...
namespace WS_Modules.GAS.GameplayAbilitySystem
{
    ...
    public sealed class GameplayAbilityCtrl : IGameplayAbilityCtrl
    {
        ...
        private readonly List<GameplayTag> activationBlockedOwnerTags = new();
        ...
        internal void InitializeActivationBlockedTags(IReadOnlyList<GameplayTag> blockedOwnerTags)
        {
            activationBlockedOwnerTags.Clear();
            if (blockedOwnerTags == null) return;

            for (int index = 0; index < blockedOwnerTags.Count; index++)
            {
                GameplayTag tag = blockedOwnerTags[index];
                if (!tag.IsValid || activationBlockedOwnerTags.Contains(tag)) continue;
                activationBlockedOwnerTags.Add(tag);
            }
        }
        #endregion
        ...
        {
            for (int index = 0; index < activationBlockedOwnerTags.Count; index++)
                if (Owner.HasTag(activationBlockedOwnerTags[index])) return true;
